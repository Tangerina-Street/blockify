...
class WebViewBlockingHelper {
    static func generateMutationObserverJS(appId: String, blockedFeatures: Set<String>) -> String {
        ...
        // --- TIMER BLOCKING (Crucial for preventing website content reload loops) ---
        let timerBlockerJS = """
        (function() {
            const noop = function() {};
            ...
            if blockedFeatures.contains("reels") {
                ...
                function blockReels() {
                    try {
                        ...
                        // Reels button in header (desktop)
            ...
            if blockedFeatures.contains("stories") {
                ...
                function blockStories() {
                    try {
                        ...
                        // Stories section in main feed area
        ...
        // --- IMPROVED TIKTOK BLOCKING LOGIC ---
        if appId == "tiktok" {
            if blockedFeatures.contains("forYouPage") || blockedFeatures.contains("following") {
                functionBodies.append("""
                function blockTikTokFeeds() {
                    try {
                        // Targets the main feed wrapper (the rotating video carousel)
                        document.querySelectorAll('[data-e2e*="feed-for-you-container"], [data-testid*="feed-container"], [role="main"]').forEach(e => e.remove());
                        
                        // Navigation links for "For You" (Home) and "Following"
                        document.querySelectorAll('a[href="/"], a[href*="/foryou"], a[href*="/following"]').forEach(e => {
                            const isForyouBlocked = \(blockedFeatures.contains("forYouPage") ? "true" : "false");
                            const isFollowingBlocked = \(blockedFeatures.contains("following") ? "true" : "false");

                            if (isForyouBlocked && (e.href.includes('/foryou') || e.href.endsWith('/'))) {
                                e.remove();
                            }
                            if (isFollowingBlocked && e.href.includes('/following')) {
                                e.remove();
                            }
                        });
                    } catch(e) {}
                ...
                """)
                calls.append("blockTikTokFeeds();")
            }
            
            if blockedFeatures.contains("discover") {
                functionBodies.append("""
                function blockTikTokDiscover() {
                    try {
                        // Targets the Discover/Search/Explore tab links
                        document.querySelectorAll('a[href*="/explore/"], a[href*="/search"]').forEach(e => e.remove());
                        // Targets the main discover page element
                        document.querySelectorAll('[data-e2e*="discover"], [data-testid*="discover"]').forEach(e => e.remove());
                    } catch(e) {}
                }
                """)
                calls.append("blockTikTokDiscover();")
            }
        }

        // --- REINFORCED FACEBOOK BLOCKING LOGIC ---
        if appId == "facebook" {
            if blockedFeatures.contains("feed") {
                functionBodies.append("""
                function blockFBFeed() {
                    // Targets the main feed role and major content containers
                    \(safe("document.querySelectorAll('[role=\"feed\"], [role=\"main\"]').forEach(e=>e.remove());"))
                    // Hides the "What's on your mind?" composer
                    \(safe("document.querySelectorAll('[role=\"region\"]').forEach(e => { if(e.innerText.includes(\"What's on your mind\")) e.remove(); });"))
                    // Target specific newsfeed IDs often used on mobile/desktop
                    \(safe("document.querySelectorAll('#MNewsFeed, #contentArea').forEach(e=>e.remove());"))
                }
                """)
            ...
            if blockedFeatures.contains("stories") {
                ...
                function blockFBStories() {
                    // Targets the container for stories/reels 
                    \(safe("document.querySelectorAll('[aria-label*=\"Stories\"], [aria-label*=\"Reels\"]').forEach(e=>e.remove());"))
                    // Targets sections with the word Stories/Reels in headers
                    \(safe("document.querySelectorAll('div, section').forEach(e => { if(e.innerText.includes('Stories') || e.innerText.includes('Reels')) e.remove(); });"))
                }
                """)
            ...
            if blockedFeatures.contains("directMessages") {
                functionBodies.append("""
                function blockFBDMs() {
                    // Targets links/icons leading to Messenger
                    \(safe("document.querySelectorAll('a[href*=\"messenger\"], a[href*=\"messages\"]').forEach(e=>e.remove());"))
                    \(safe("document.querySelectorAll('[aria-label*=\"Messenger\"]').forEach(e=>e.remove());"))
                    // Hides the chat side panel
                    \(safe("document.querySelectorAll('[role=\"complementary\"]').forEach(e => { if(e.innerText.includes('Chats')) e.remove(); });"))
                }
                """)
        ...
        if calls.isEmpty {
            ...
            (function() {
                ...
                try {
                    ...
                    // Use a single, persistent MutationObserver for dynamic content changes
        ...
        // Combine the timer blocker and the observer/feature blocker
        return timerBlockerJS + "\n" + observerJS
    }
}
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 12) {
                ...
                Spacer()
                // Action directly sets showWeb to true
...
struct BlockedWebViewView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> WKWebView {
        // 1. Get the JS code for blocking timers and features
        guard let blocked = settings.blockedFeatures[appId] else {
        ...
        let jsString = WebViewBlockingHelper.generateMutationObserverJS(appId: appId, blockedFeatures: blocked)

        // 2. Create a WKUserScript to inject the code at the START of the document
        let script = WKUserScript(
            source: jsString,
            injectionTime: .atDocumentStart, // This is crucial for blocking timers
            forMainFrameOnly: true
        )

        // 3. Configure the WebView to use the script
    ...
    class Coordinator: NSObject, WKNavigationDelegate {
        ...
        // No need to inject JS here anymore
        func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
            // JS injection is handled at document start via WKUserScript
        }
    }
}
